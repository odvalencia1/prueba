...
class Notadecredito(Document):
	def on_submit(self):
		...
		if self.docstatus == 1:
			...
			for i in range(1, le):
				secuencial += "0"
			secuencial +=str(int(negocio.secuencial)+1)

@frappe.whitelist(allow_guest=True)
def getProductos(factura):
	return frappe.db.get_list('detalle_factura', filters={"parent":factura},fields=("name", "producto","cantidad","precio_unitario","total","nombre","precio_impuesto"))
